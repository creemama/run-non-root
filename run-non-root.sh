...
print_help () {
  ...
  echo "                          default is sh."
  echo "  RUN_NON_ROOT_GID        The group ID to use when executing the command; the"
  echo "                          default is the first unused group ID strictly less"
  echo "                          than 1000; this variable is ignored if we are already"
  echo "                          running as a non-root user; the -g and --gid options"
  echo "                          override this environment variable."
  echo "  RUN_NON_ROOT_GROUP      The group name to use when executing the command; the"
  echo "                          default is nonroot; this variable is ignored if we"
  echo "                          are already running as a non-root user; the -f and"
  echo "                          --group options override this environment variable."
  echo "  RUN_NON_ROOT_UID        The user ID to use when executing the command; the"
  echo "                          default is the first unused user ID strictly less"
  echo "                          than 1000; this variable is ignored if we are already"
  echo "                          running as a non-root user; the -u and --uid options"
  echo "                          override this environment variable."
  echo "  RUN_NON_ROOT_USER       The username to use when executing the command; the"
  echo "                          default is nonroot; this option is ignored if we are"
  echo "                          already running as a non-root user; the -t and --user"
